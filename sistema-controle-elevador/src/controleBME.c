...
struct bme280_dev dev;  // Definição global da estrutura dev
...
int8_t user_i2c_read(uint8_t reg_addr, uint8_t *data, uint32_t len, void *intf_ptr)
{
    ...
    wiringPiI2CWrite(id.fd, reg_addr);
    read(id.fd, data, len);
    return 0;
}
...
struct bme280_data read_and_print_temperature(struct bme280_dev *dev)
{
    ...
    /* Structure to store and return the pressure, temperature and humidity values */
    struct bme280_data comp_data;
    ...
    if (rslt != BME280_OK)
    {
        ...
        // You may want to handle the error or return a special value for failure
        comp_data.temperature = comp_data.pressure = comp_data.humidity = 0.0;
        return comp_data;
    }
    ...
    dev->delay_us(200000, dev->intf_ptr);  // Wait for 1 second (adjust as needed)
